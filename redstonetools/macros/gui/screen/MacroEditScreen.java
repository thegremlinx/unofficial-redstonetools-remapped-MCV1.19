...
public class MacroEditScreen extends GameOptionsScreen {
    ...
    public void init() {
        ...
        if ( KeyBindingUtils.isKeyAlreadyBound(keyCode) ) { text = new LiteralTextContent(text.getString()).formatted(Formatting.RED); }

        keyBindButton = new ButtonWidget(this.width / 2 + 26, 55, 75, 20, text, (button) -> {
            detectingKeycodeKey = true;
            keyBindButton.setMessage((new LiteralTextContent("> ")).append(keyBindButton.getMessage().copy().formatted(Formatting.YELLOW)).append(" <").formatted(Formatting.YELLOW));
        });
    ...
    private boolean updateKeybinding(Key key) {
        if (detectingKeycodeKey) {
            ...
            if ( KeyBindingUtils.isKeyAlreadyBound(key) ) { text = new LiteralTextContent(text.getString()).formatted(Formatting.RED); }
